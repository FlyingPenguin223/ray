...
/*
            ...
            g = (pixel & 0x00FF0000) >> (8*2);
            b = (pixel & 0x0000FF00) >> (8*1);
...
/*            int darkness = darknessThreshold > 0 ? (cur.dist * cur.dist * 255 / darknessThreshold) : 0;
            Uint8 dark = 255 - min(darkness, 255);

            SDL_SetTextureColorMod(getEntityTexture(cur.id), dark, dark, dark); */
